...
# Create a helper function to address the fourth vault node
function vault_5 {
    (export VAULT_ADDR=http://127.0.0.5:8200 && vault "$@")
}
...
function stop {
  case "$1" in
    ...
    vault_5)
      ...
      ;;
    all)
      for vault_node_name in vault_1 vault_2 vault_3 vault_4 vault_5 ; do
        stop_vault $vault_node_name
      done
      ;;
    *)
      printf "\n%s" \
        "Usage: $script_name stop [all|vault_1|vault_2|vault_3|vault_4|vault_5]" \
        ""
      ;;
    esac
}
...
function clean {
  ...
  for loopback_address in "127.0.0.2" "127.0.0.3" "127.0.0.4" "127.0.0.5" ; do
  ...
  for config_file in $demo_home/config-vault_1.hcl $demo_home/config-vault_2.hcl $demo_home/config-vault_3.hcl $demo_home/config-vault_4.hcl $demo_home/config-vault_5.hcl ; do
  ...
  for raft_storage in $demo_home/raft-vault_2 $demo_home/raft-vault_3 $demo_home/raft-vault_4 $demo_home/raft-vault_5 ; do
  ...
  for vault_log in $demo_home/vault_1.log $demo_home/vault_2.log $demo_home/vault_3.log $demo_home/vault_4.log $demo_home/vault_5.log ; do
    if [[ -f "$vault_log" ]] ; then
      printf "\n%s" \
...
function create_network {
  ...
  case "$os_name" in
    darwin)
      ...
      printf "\n%s" \
        "[vault_5] Enabling local loopback on 127.0.0.5 (requires sudo)" \
        ""
    ...
    linux)
      ...
      sudo ip addr add 127.0.0.5/8 dev lo label lo:3
...
EOF
  ...
  listener "tcp" {
    address = "127.0.0.5:8200"
    cluster_address = "127.0.0.5:8201"
    tls_disable = true
  }
  seal "transit" {
    address            = "http://127.0.0.1:8200"
  ...
  disable_mlock = true
  cluster_addr = "http://127.0.0.5:8201"
EOF
  printf "\n"
}
...
function setup {
  case "$1" in
    ...
    vault_5)
      ...
      ;;
    all)
      for vault_setup_function in setup_vault_1 setup_vault_2 setup_vault_3 setup_vault_4 setup_vault_5 ; do
    ...
    *)
      ...
      "Usage: $script_name setup [all|vault_1|vault_2|vault_3|vault_4|vault_5]" \
      ""
      ;;
  esac
}
...
case "$1" in
  ...
  *)
    printf "\n%s" \
      ...
      "" \
      "Usage: $script_name [create|setup|status|stop|clean|vault_1|vault_2|vault_3|vault_4|vault_5]" \
      ""
    ;;
esac
